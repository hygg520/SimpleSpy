...
function newRemote(type, data)
    ...
    local connect = Button.MouseButton1Click:Connect(function()
        ...
        if blocked then
            log.GenScript = "-- 此远程已被 SimpleSpy 阻止发送到服务器\n\n" .. log.GenScript
        end
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
    ...
    for k, v in next, t do -- iterates over table
        ...
        if size > (getgenv().SimpleSpyMaxTableSize or 1000) then
            s = s .. "\n" .. string.rep(" ", l) .. "-- 已达到最大表大小，修改 'getgenv().SimpleSpyMaxTableSize' 可调整上限 "
            break
        end
...
function f2s(f)
    ...
    if configs.funcEnabled then
        ...
        if funcname and funcname:match("^[%a_]+[%w_]*$") then
            return `function {funcname}() end -- 调用的函数：{funcname}`
        end
    end
    return tostring(f)
end
...
function formatstr(s, indentation)
    ...
    return '"' .. handled .. '"' .. (reachedMax and " --[[ 已达到最大字符串大小，修改 'getgenv().SimpleSpyMaxStringSize' 可调整上限 ]]" or "")
end
...
function()
    ...
    if func then
        ...
        if typeoffunc ~= 'string' then
            ...
            info = {
                info = getinfo(func),
                constants = lclosure and deepclone(getconstants(func)) or "N/A --预期为 Lua 闭包，实际为 C 闭包",
            ...
            if configs.advancedinfo then
                ...
                info["advancedinfo"] = {
                    ...
                    Protos = lclosure and getprotos(func) or "N/A --预期为 Lua 闭包，实际为 C 闭包"
                }
...
newButton(
    "排除 (i)",
    function() return "点击排除此远程。\n排除后 SimpleSpy 会忽略它，但仍可正常使用。" end,
    function()
        if selected then
...
newButton(
    "阻止 (i)",
    function() return "点击阻止此远程触发。\n阻止远程不会将其从 SimpleSpy 日志中移除，但不会再发送到服务器。" end,
    function()
        if selected then
...
newButton(
    "禁用信息",
    function() return string.format("[%s] 切换函数信息（某些游戏中可能导致卡顿）", configs.funcEnabled and "已启用" or "已禁用") end,
    function()
        configs.funcEnabled = not configs.funcEnabled
        TextLabel.Text = string.format("[%s] 切换函数信息（某些游戏中可能导致卡顿）", configs.funcEnabled and "已启用" or "已禁用")
    end
)

newButton(
    "自动阻止",
    function() return string.format("[%s] [BETA] 智能检测并排除日志中刷屏的远程调用", configs.autoblock and "已启用" or "已禁用") end,
    function()
        configs.autoblock = not configs.autoblock
        TextLabel.Text = string.format("[%s] [BETA] 智能检测并排除日志中刷屏的远程调用", configs.autoblock and "已启用" or "已禁用")
        history = {}
        excluding = {}
    end
)

newButton("记录本机调用",function()
    return ("[%s] 记录客户端触发的远程"):format(configs.logcheckcaller and "已启用" or "已禁用")
end,
function()
    configs.logcheckcaller = not configs.logcheckcaller
    TextLabel.Text = ("[%s] 记录客户端触发的远程"):format(configs.logcheckcaller and "已启用" or "已禁用")
end)
